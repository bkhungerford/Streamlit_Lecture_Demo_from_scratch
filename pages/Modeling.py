...
model_name = st.sidebar.selectbox('Select Model', ['random_forest', 'logistic_regression'], index=0)
model = load_model_ml(FPATHS['models'][model_name])

# Evaluate model subheader and button
st.sidebar.subheader('Evaluation')

# Define the labels for the classification metrics
...
# Now we can take values for the features and produce a prediction--True deployment of the model
st.sidebar.subheader('Make a Prediction')

# Feature inputs
# Every feature must be included--this can take some time--consider adding step = arguments
